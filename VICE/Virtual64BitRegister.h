...
namespace VICE
{
  namespace Memory
  {
    // The Virtual64BitRegister implements a 64-bit universal (shift-left/shift-right
    ...
    // Virtual64BitRegister takes during update. The following truth table shows the
    ...
    class Virtual64BitRegister : public VICE::Component::VirtualComponent
    {
    private:
      ////////////////////
      // sub-components //
      ...
      VICE::Component::VirtualFlipFlop Bits[64];
      // input/action controllers
      VICE::Control::VirtualMultiplexer_4_1 BitControl[64];
      // inverts the output of BitControl; necessary because of the
      // way VirtualFlipFlops set bits. 
      VICE::Logic::VirtualInverter MuxInverters[64]; 
    public:
      /////////////
      // Methods //
      /////////////
    ...
    public:
      ...
      // sets the Input bits as an array of VirtualConnections (size 64)
      // NOTE: This method considers input to be an array of const 
      // VirtualConnections of size 64. Unexpected results can occur
      // if an array of a smaller size is passed. 
      void Set_Input(const VICE::Component::VirtualConnection * input);
      // sets the Input lines as an array of VirtualConnections (size 64)
      // NOTE: This method considers input to be an array of pointers to
      // const VirtualConnections of size 64. Unexpected results can occur
      // if an array of a smaller size is passed. 
      void Set_Input(const VICE::Component::VirtualConnection ** input);
    ...
    public:
      ...
      //*********************************************************//
      //** Output -- the 64-bit current value of the register  **//
      //**   Each bit defaults to VirtualConnection::AlwaysOff **//
      //*********************************************************//
    private:      
      const VICE::Component::VirtualConnection * Output[64];
    public:
      // returns the specified bit VirtualConnection for the Sum output
      const VICE::Component::VirtualConnection * Get_Output(int bit);
      // returns the Sum output as an array of VirtualConnections (size 64)
